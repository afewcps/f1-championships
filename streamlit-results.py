    ...
    if response.status_code == 200:
        ...
        for result in results:
            if result.get("object") == "database" and f"{gp_name} GP {year}" in result.get("title", [{}])[0].get("text", {}).get("content", ""):
                st.success(f"Found existing database for {gp_name} GP {year}")
                return result["id"]
...
def create_gp_database(gp_name, year, headers, block_id):
    ...
    if response.status_code == 200:
        ...
        st.success(f"Created new database for {gp_name} GP {year}")
        ...
        if view_response.status_code == 200:
            st.success(f"Updated database title for {gp_name} GP {year}")
        else:
            st.error(f"Failed to update database title: {view_response.status_code}")
            st.code(view_response.text)
        
        return database_id
    else:
        st.error(f"Failed to create database: {response.status_code}")
...
def process_race_weekend(year, gp_name, is_sprint_weekend, headers, block_id):
    ...
    status_text.text("Finding or creating database...")
    ...
    if not gp_database_id:
        status_text.text(f"No existing database found for {gp_name} GP {year}. Creating new one...")
        gp_database_id = create_gp_database(gp_name, year, headers, block_id)
    
    if not gp_database_id:
        status_text.text("Failed to create or find GP database.")
        st.error("Failed to create or find GP database.")
        return
